    ...
    if (characters[chid]?.data?.character_book) {
        ...
        if (!accountStorage.getItem(checkKey) && (!worldName || !world_names.includes(worldName))) {
            ...
            if (power_user.world_import_dialog) {
                const html = `<h3>此角色包含嵌入的世界设定/知识库。</h3>
                <h3>您是否想要现在导入？</h3>
                <div class="m-b-1">如果您想稍后导入，请在角色面板的"更多..."下拉菜单中选择"导入角色卡知识"。</div>`;
                const checkResult = (result) => {
                    if (result) {
                        importEmbeddedWorldInfo(true);
                    }
                };
                callGenericPopup(html, POPUP_TYPE.CONFIRM, '', { okButton: '是' }).then(checkResult);
            }
            else {
                toastr.info(
